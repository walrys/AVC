...
if __name__ == '__main__':
    ...
    for filename in os.listdir(directory):
        ...
        if filename.endswith(".mp4"):            
            #print "1. Set the access path to the original video clip."
            video_file = filename
            
            #print  "2. Open the video clip."
            vidcap = cv2.VideoCapture(video_file)

            #print "3. Get and store the resulting frames via the specific path."
            store_frame_path="./"+video_file[:-4]+"-"
            keyframes = getKeyFrames(vidcap=vidcap, store_frame_path=store_frame_path)

            #print "4. Close the video clip."
            vidcap.release()
